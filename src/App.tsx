...
function App() {
  ...
  const [dataState, setDataState] = useState<'loading' | 'connected' | 'mixed'>('loading');
  ...
  const sourceLabel = dataState === 'connected' ? 'BACKEND CONNECTED' : dataState === 'loading' ? 'SYNCING DATA' : 'MIXED / FALLBACK';
  const sourceColor = dataState === 'connected' ? '#10b981' : dataState === 'loading' ? '#38b6ff' : '#f59e0b';
